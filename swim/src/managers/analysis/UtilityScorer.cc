 ...
 */
double UtilityScorer::getAccruedUtility(const Model& model, const ServerModel& serverModel, const EnvironmentModel& environmentmodel, const SystemModel& systemModel)
{
    ...
    double throughput = (environmentmodel.getArrivalMean() > 0) ? (1 / environmentmodel.getArrivalMean()) : 0.0;

    double brownoutFactor = model.brownoutLevelToFactor(serverModel.getBrownOutLevel());

    double positiveUtility = round((throughput * (brownoutFactor * brownoutRevenue + (1 - brownoutFactor) * normalRevenue)));

    double utility = ((systemModel.avgResponseTime>RT_THRESHOLD || systemModel.avgResponseTime < 0) ? std::min(0.0, throughput * normalRevenue - latePenalty) : positiveUtility);
