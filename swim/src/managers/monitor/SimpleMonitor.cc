...
const char *SimpleMonitor::SIG_ANALYSIS_EVALUATION = "analysisEvaluation";
...
{
    SystemModel systemModel = pProbe->getUpdatedSystemModel();
    pModel->setSystemModel(systemModel);

    // std::cout << " max server in monitor: " << pModel->getMaxServers() << std::endl;

    const EnvironmentModel &env = pModel->getEnvironmentModel();
    ...
    double periodUtility = UtilityScorer::getAccruedUtility(*pModel,
                                                            pModel->getServerModel(), pModel->getEnvironmentModel(), pModel->getSystemModel()) *
                           pModel->getEvaluationPeriod();
    emit(utilitySignal, periodUtility);

    emit(analysisEvaluationSignal, true);
}

void SimpleMonitor::oversamplingHandler()
{
    EnvironmentModel environment = pProbe->getUpdatedEnvironmentModel();
    pModel->setEnvironmentModel(environment);
}
