...
SystemModel SimProbe::getUpdatedSystemModel() {
    SystemModel systemModel;
    systemModel.utilization = 0;

    for (auto& entry : utilization) {
        systemModel.utilization += entry.second.getPercentageAboveZero();
    }

    systemModel.basicResponseTime = getBasicResponseTime();
    systemModel.basicThroughput = getBasicThroughput();
    systemModel.optResponseTime = getOptResponseTime();
    systemModel.optThroughput = getOptThroughput();
...
EnvironmentModel SimProbe::getUpdatedEnvironmentModel() {
