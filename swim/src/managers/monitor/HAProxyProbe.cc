...
SystemModel HAProxyProbe::getUpdatedSystemModel() {

    // update observations only if not stale
    auto currentTime = boost::posix_time::microsec_clock::local_time();
    if (!lastSystemModelUpdate.is_not_a_date_time()
            && (currentTime - lastSystemModelUpdate).total_milliseconds()
                    <= MEASUREMENT_OBSOLECENSE_MSEC) {
        return systemModel;
    }

    lastSystemModelUpdate = currentTime;
    ...
    ASSERT(valueStr == "2");
    getline(responseStream, valueStr, ',');
    systemModel.basicThroughput = stod(valueStr);
    getline(responseStream, valueStr, ',');
    systemModel.basicResponseTime = stod(valueStr) / 1000;
    getline(responseStream, valueStr, ',');
    systemModel.optThroughput = stod(valueStr);
    if (!getline(responseStream, valueStr, ',')) {
        error("response from LogFileProbe does not match expected format");
    }
    systemModel.optResponseTime = stod(valueStr) / 1000;

    systemModel.avgResponseTime = (systemModel.basicResponseTime * systemModel.basicThroughput + systemModel.optResponseTime * systemModel.optThroughput)
            / (systemModel.basicThroughput + systemModel.optThroughput);

    // get total utilization
    systemModel.utilization = 0;
    tcpStream << "util" << endl;
    getline(tcpStream, response);
    ...
    while (server <= activeServers && getline(responseStream, valueStr, ',')) {
        ...
        systemModel.utilization += serverUtilization;
        server++;
    }
    return systemModel;
}

EnvironmentModel HAProxyProbe::getUpdatedEnvironmentModel() {
    ...
    if (lastEnvironmentUpdate.is_not_a_date_time()
        ...
        environmentModel.setArrivalMean(measuredMeanInterArrival);
        environmentModel.setArrivalVariance(pow(measuredMeanInterArrival, 2)); // assume exponential distribution
    }
...
double HAProxyProbe::getBasicResponseTime() {
    return getUpdatedSystemModel().basicResponseTime;
}

double HAProxyProbe::getOptResponseTime() {
    return getUpdatedSystemModel().optResponseTime;
}

double HAProxyProbe::getBasicThroughput() {
    return getUpdatedSystemModel().basicThroughput;
}

double HAProxyProbe::getOptThroughput() {
    return getUpdatedSystemModel().optThroughput;
}

double HAProxyProbe::getUtilization(const std::string& serverName) {
    getUpdatedSystemModel();
...
double HAProxyProbe::getArrivalRate() {
    double arrivalMean = getUpdatedEnvironmentModel().getArrivalMean();
    double rate = 0;
    if (arrivalMean > 0) {
