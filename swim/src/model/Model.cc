...
ServerModel Model::getServerModel()
{
    ...
    serverModel.setBrownOutLevel(brownoutFactorToLevel(brownoutFactor));
    serverModel.setActiveServers(activeServers);
    if (events.empty())
    {
        serverModel.setBootRemain(0);
    }
    else
    {
        ...
        if (eventIt != events.end())
        {
             ...
             */
            serverModel.setBootRemain(std::max(1, bootRemain));
            eventIt++;
            ASSERT(eventIt == events.end()); // only one tactic should be active
        }
    }
...
const EnvironmentModel &Model::getEnvironmentModel() const
{
    return environmentModel;
}

void Model::setEnvironmentModel(const EnvironmentModel &environmentModel)
{
...
{
    // std::cout << " max server in model: " << maxServers << std::endl;
    return maxServers;
}
...
const SystemModel &Model::getSystemModel() const
{
    return systemModel;
}

void Model::setSystemModel(const SystemModel &systemModel)
{
